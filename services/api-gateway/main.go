...
func main() {
	...
	r.PUT("/api/accounts/:accountId/limits", middleware.RequireRole("READ"), handlers.UpdateAccountLimits(accountClient, emailClient, authClient, clientClient))
	...
	r.POST("/client/forgot-password", rlAuth, handlers.ClientForgotPassword(authClient, emailClient))
	r.POST("/client/reset-password", rlAuth, handlers.ClientResetPassword(authClient))
	r.POST("/auth/logout", rlAuth, handlers.Logout(authClient))
	r.POST("/auth/totp/generate", middleware.RequireRole("READ", "WRITE", "SUPERVISOR", "AGENT", "CLIENT"), handlers.TOTPGenerate(authClient))
	r.POST("/auth/totp/verify", middleware.RequireRole("READ", "WRITE", "SUPERVISOR", "AGENT", "CLIENT"), handlers.TOTPVerify(authClient))
	r.POST("/auth/totp/validate", handlers.TOTPValidateLogin(authClient))
	r.DELETE("/auth/totp", middleware.RequireRole("READ", "WRITE", "SUPERVISOR", "AGENT", "CLIENT"), handlers.TOTPDisable(authClient))
	r.GET("/notifications", middleware.RequireRole("READ", "SUPERVISOR", "AGENT", "CLIENT"), handlers.ListNotifications(authClient))
	r.GET("/notifications/unread-count", middleware.RequireRole("READ", "SUPERVISOR", "AGENT", "CLIENT"), handlers.GetUnreadCount(authClient))
	r.PUT("/notifications/:id/read", middleware.RequireRole("READ", "SUPERVISOR", "AGENT", "CLIENT"), handlers.MarkNotificationRead(authClient))
	r.PUT("/notifications/read-all", middleware.RequireRole("READ", "SUPERVISOR", "AGENT", "CLIENT"), handlers.MarkAllNotificationsRead(authClient))
	...
	r.GET("/client/orders", rlOrders, handlers.GetMyOrders(orderClient, "CLIENT"))
	...
	r.GET("/orders/my", rlOrders, middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.GetMyOrders(orderClient, "EMPLOYEE"))
	...
	// Watchlists (BE-C3-04) — /watchlists/quick must be before /watchlists/:id
	r.GET("/watchlists/quick", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.GetQuickWatchlist(portfolioClient))
	r.POST("/watchlists", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.CreateWatchlist(portfolioClient))
	r.GET("/watchlists", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.ListWatchlists(portfolioClient))
	r.DELETE("/watchlists/:id", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.DeleteWatchlist(portfolioClient))
	r.POST("/watchlists/:id/items", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.AddWatchlistItem(portfolioClient))
	r.DELETE("/watchlists/:id/items/:listing_id", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.RemoveWatchlistItem(portfolioClient))
	r.GET("/watchlists/:id/items", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.GetWatchlistItems(portfolioClient))

	// Audit logs (BE-C3-05)
	r.GET("/audit-logs", middleware.RequireRole("ADMIN", "SUPERVISOR"), handlers.ListAuditLogs(employeeClient))

	// Recurring orders (BE-C3-06)
	r.POST("/recurring-orders", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.CreateRecurringOrder(orderClient))
	r.GET("/recurring-orders", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.ListRecurringOrders(orderClient))
	r.PUT("/recurring-orders/:id/pause", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.PauseRecurringOrder(orderClient))
	r.PUT("/recurring-orders/:id/resume", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.ResumeRecurringOrder(orderClient))
	r.DELETE("/recurring-orders/:id", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.CancelRecurringOrder(orderClient))

	// Price alerts (BE-C3-02)
	r.POST("/price-alerts", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.CreatePriceAlert(securitiesClient))
	r.GET("/price-alerts", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.ListPriceAlerts(securitiesClient))
	r.DELETE("/price-alerts/:id", middleware.RequireRole("AGENT", "SUPERVISOR", "CLIENT"), handlers.DeletePriceAlert(securitiesClient))

	// Dividend history (BE-C3-07)
	r.GET("/dividends/my", middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.GetDividendHistory(portfolioClient, "EMPLOYEE"))
	r.GET("/dividends/my/:listing_id", middleware.RequireRole("AGENT", "SUPERVISOR"), handlers.GetDividendHistory(portfolioClient, "EMPLOYEE"))
	r.GET("/client/dividends/my", handlers.GetDividendHistory(portfolioClient, "CLIENT"))
	r.GET("/client/dividends/my/:listing_id", handlers.GetDividendHistory(portfolioClient, "CLIENT"))
